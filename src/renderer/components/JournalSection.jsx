...
export default function JournalSection({ journalFiles, activeFile, onOpenFile, onCreateFile, collapsed, toggleCollapse, search, onContextMenu, onTodayJournal }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid var(--glass-border)' }}>
      {/* Journal root row */}
      <div className="flex items-center gap-1 mx-1 px-1.5 py-1.5 rounded-md my-px transition-all duration-150">
        <span
        ...
        {onTodayJournal && (
          <button
            onClick={e => { e.stopPropagation(); onTodayJournal() }}
            title="Open or create today's journal"
            className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-md transition-all duration-150 flex-shrink-0"
